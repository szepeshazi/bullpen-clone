...
Hint? nakedSetRule(HintContext ctx) =>
...
Hint? _firstExcludable(
  ...
  for (final i in subset) {
    for (var j = 0; j < ctx.size; j++) {
      ...
      if (containedPenIds.contains(ctx.board.cellAt(r, c).penId)) {
        continue;
      }
      if (ctx.essential.contains((r, c))) {
        continue;
      }
      return Hint(
        row: r,
...
String _reason(_Axis axis, List<int> subset, Set<int> containedPenIds) {
  ...
  // ignore: prefer_single_quotes -- string contains a literal apostrophe
  const otherPens = "other pens’";
  return '$groupWord $labels must supply all bulls for '
      '$nPens $penWord fully within $groupRef '
      '— no room for $otherPens bulls in $groupPhrase';
}
